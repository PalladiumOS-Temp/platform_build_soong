...
type variableProperties struct {
	Product_variables struct {
		...
		// include palladium variables
		Palladium android.Product_variables
	} `android:"arch_variant"`
}
...
type productVariables struct {
	...
	// include palladium variables
	Palladium android.ProductVariables
}
...
func createVariablePropertiesType(moduleTypeProps []interface{}, productVariables interface{}) reflect.Type {
	typ, _ := proptools.FilterPropertyStruct(reflect.TypeOf(productVariables),
		func(field reflect.StructField, prefix string) (bool, reflect.StructField) {
			if strings.HasPrefix(prefix, "Product_variables.Palladium") {
				// Convert Product_variables.Palladium.Foo to Palladium.Foo
